...
key.shadow.camera.top=5; key.shadow.camera.bottom=-2.5;
key.shadow.radius=2;
scene.add(key);
...
var gndMat = new THREE.ShadowMaterial({opacity:0.22});
var gnd = new THREE.Mesh(new THREE.PlaneGeometry(14,18), gndMat);
gnd.rotation.x=-PI/2; gnd.position.y=-0.95; gnd.receiveShadow=true;
scene.add(gnd);
...
var extSettings = {
  ...
  bevelThickness: 0.014, bevelSize: 0.012, bevelSegments: 6
};
...
function makeTruck(tz){
  ...
  bp.position.y=-0.09; bp.castShadow=true; tg.add(bp);
  // Kingpin nut
...
function makeWheel(x,z){
  ...
  // Main urethane wheel — smooth CylinderGeometry
  var wh=new THREE.Mesh(new THREE.CylinderGeometry(0.285,0.285,0.245,28), mWheel);
  wh.rotation.z=PI/2; wh.castShadow=true; wg.add(wh);
  // Hub insert (slightly smaller, cream/off-white)
  var hub=new THREE.Mesh(new THREE.CylinderGeometry(0.088,0.088,0.26,14), mHub);
  hub.rotation.z=PI/2; wg.add(hub);
  // Bearing (dark metal ring)
  var brg=new THREE.Mesh(new THREE.CylinderGeometry(0.046,0.046,0.27,10), mHard);
  brg.rotation.z=PI/2; wg.add(brg);
  // Subtle wheel edge highlight rings
  [-0.11,0.11].forEach(function(ox){
    var ring=new THREE.Mesh(new THREE.TorusGeometry(0.284,0.008,6,28), mWheel);
    ring.rotation.y=PI/2; ring.position.x=ox; wg.add(ring);
  });
...
function animate(){
  ...
  cP=lerp(cP,tP,0.16); cR=lerp(cR,tR,0.16);
